...
export function QuantityStepper({ value, onChange, max, size = "lg", label = "Quantité" }: QuantityStepperProps) {
  ...
  return (
    <div role="group" aria-label={label} className={`inline-flex ${height} items-center border border-[var(--som-border-strong)]`}>
      <button type="button" onClick={() => onChange(value - 1)} disabled={value <= 1} aria-label="Diminuer la quantité" className={button}>
