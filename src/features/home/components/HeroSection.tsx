...
import { HERO_DEFAULTS, HERO_OVERLAYS as OVERLAYS } from "../constants";
...
export function HeroSection({ data }: { data: HeroBannerContent | null }) {
  ...
  const layout = toHeroLayout(data?.layout);
  ...
  return (
    <>
      {layout === "split" ? (
        ...
        <section className="relative hidden h-[88vh] max-h-[880px] min-h-[600px] overflow-hidden bg-[var(--som-primary-100)] md:block">
          <HeroMedia {...media} priority />
          <div aria-hidden className={`absolute inset-0 ${OVERLAYS[layout]}`} />
          <div
            className={`absolute inset-0 flex px-[clamp(40px,6vw,96px)] ${
              layout === "centered" ? "items-center justify-center text-center" : "items-end pb-[clamp(48px,8vh,96px)]"
            }`}
          >
            <div className={layout === "centered" ? "max-w-[720px]" : "max-w-[620px]"}>
              <p className="m-0 mb-6 text-[11.5px] uppercase tracking-[0.3em] text-white/85">{hero.eyebrow}</p>
              <h1 className="m-0 text-[clamp(40px,4.6vw,76px)] font-medium leading-[1.04] tracking-[-0.01em] text-white">
                {hero.title} <em className="font-light italic">{hero.titleHighlight}</em> {hero.titleSuffix}
              </h1>
              <p className="mb-10 mt-6 text-[17px] font-light leading-[1.65] text-white/85">{hero.description}</p>
              <Link href={hero.buttonLink} className="btn-secondary-light">
                {hero.buttonText}
              </Link>
            </div>
          </div>
