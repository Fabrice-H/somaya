...
export function PasswordInput(props: PasswordInputProps) {
  ...
  return (
    <div className="input-group-som">
      <input {...props} type={visible ? "text" : "password"} className="input-som" />
      <button
        type="button"
        onClick={() => setVisible((value) => !value)}
        aria-label={visible ? "Masquer le mot de passe" : "Afficher le mot de passe"}
        className="flex h-10 w-10 shrink-0 cursor-pointer items-center justify-center text-[var(--som-gray)] hover:text-[var(--som-ink)]"
      >
